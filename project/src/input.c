...
/**
 * Checks the format of the input read by scanf.
...
*/
static int check_format(int scanf_nbr, t_lists *lists)
{
	if (scanf_nbr > 0)
		return 0;
...
/**
 * Reads an integer from the user with a prompt and error message.
...
int read_int(char *prompt, char *err, int limit_down, int limit_up, t_lists *lists)
{
	...
	do
	{
		...
		if (check_format(scanf_nbr, lists) == 1)
			continue;
